'use strict'
import { getImageFiles } from './utils/functions.js'
...
function registerLocalResourceProtocol() {
  protocol.registerFileProtocol('img', (request, callback) => {
    ...
    // Decode URL to prevent errors when loading filenames with UTF-8
    // chars or chars like "#"
    const decodedUrl = decodeURI(url) // Needed in case URL contains
    // spaces
    try {
...
ipcMain.on('open-file', (event) => {
  ...
  let string = dialog.showOpenDialogSync(win, {
    title: "Open an image",
    properties: ["openFile"],
    filters: [
      {name: "Fichiers images", extensions: ['jpg', 'jpeg', 'png']}
    ]
  })
  if (string !== undefined)
